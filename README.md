...
./run_celltype_enrichment_v1_4.sh \
  ...
  --specificity-mode penalize \
```
Then I used estimate_celltype counts to estimate number of cell types each gene is expressed in based on the specificity value
...
Example usage
```
./run_universal_plot_maker_with_options.sh --file enrichV1_4_3clusters_tau_only.tsv --out enrichV1_4_3clusters.html
```
...
Therefore I can't use raw values for filteration as it might be biased towards rows with a higher number of read counts. I can control the weight I have towards this by controlling alpha in the following equation. If alpha=0, there is no affect from the weighted values, if alpha is 1, it uses full read‐fraction weighting. I used alpha value 0.5 to only filter out extreme values.
